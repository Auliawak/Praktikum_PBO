...
public class DealerManager {
    ...
    public ArrayList<Mobil> cariMobil(String kata, boolean cariByNama) {
        ...
        for (Mobil m : this.daftarMobil) {
            String target = cariByNama ? m.getNama() : m.getTipe();
            if (target.toLowerCase().contains(kata.toLowerCase())) {
                hasil.add(m);
            }
        }
        return hasil;
    ...
    public ArrayList<Mobil> cariMobil(int tahunDari, int tahunSampai) {
        ArrayList<Mobil> hasil = new ArrayList<Mobil>();
        for (Mobil m : this.daftarMobil) {
            if (m.getTahun() >= tahunDari && m.getTahun() <= tahunSampai) {
                hasil.add(m);
            }
        }
        return hasil;
