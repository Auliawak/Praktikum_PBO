import java.util.ArrayList;
import java.util.Scanner;

public class Main {

    private Scanner       scanner;
    private InputManager  input;
    private DealerManager dealer;

    public Main() {
        this.scanner = new Scanner(System.in);
        this.input   = new InputManager(scanner);
        this.dealer  = new DealerManager();
    }

    public static void main(String[] args) {
        new Main().jalankan();
    }

    private void jalankan() {
        tampilHeader();

        boolean jalan = true;
        while (jalan) {
            tampilMenu();
            int pilihan = input.bacaInt("  Pilih menu [0-5]: ");
            System.out.println();

            switch (pilihan) {
                case 1 -> menuTambah();
                case 2 -> menuLihat();
                case 3 -> menuUbah();
                case 4 -> menuHapus();
                case 5 -> menuSimulasi();
                case 0 -> {
                    System.out.println("  Terima kasih telah menggunakan Sistem Dealer Honda!");
                    System.out.println("  Program selesai. Sampai jumpa!");
                    System.out.println();
                    jalan = false;
                }
                default -> System.out.println("  [!] Pilihan tidak valid. Masukkan angka 0-5.\n");
            }
        }

        scanner.close();
    }

    private void tampilHeader() {
        System.out.println("==========================================================");
        System.out.println("      SISTEM MANAJEMEN DEALER MOBIL HONDA");
        System.out.println("      Pemrograman Berorientasi Objek - Posttest 4");
        System.out.println("      Polymorphism");
        System.out.println("==========================================================");
        System.out.println();
    }

    private void tampilMenu() {
        System.out.println("----------------------------------------------------------");
        System.out.println("  MENU UTAMA");
        System.out.println("----------------------------------------------------------");
        System.out.println("  1. Tambah Data Mobil");
        System.out.println("  2. Lihat Semua Mobil");
        System.out.println("  3. Ubah Data Mobil");
        System.out.println("  4. Hapus Data Mobil");
        System.out.println("  5. Simulasi Pajak & Diskon");
        System.out.println("  0. Keluar");
        System.out.println("----------------------------------------------------------");
    }

    private void garis() {
        System.out.println("----------------------------------------------------------");
    }

    private void menuTambah() {
        System.out.println("  [TAMBAH DATA MOBIL]");
        garis();

        System.out.println("  Pilih kategori:");
        System.out.println("  1. Mobil Bensin");
        System.out.println("  2. Mobil Listrik");
        int kategori = input.bacaInt("  Pilih [1/2]: ");
        System.out.println();

        if (kategori != 1 && kategori != 2) {
            System.out.println("  [!] Pilihan tidak valid.\n");
            return;
        }

        String id = dealer.generateIdBaru();
        System.out.println("  ID Otomatis : " + id);
        System.out.println();

        System.out.print("  Nama Model  : ");
        String nama = input.bacaString();

        System.out.print("  Tipe Mobil  : ");
        String tipe = input.bacaString();

        System.out.print("  Warna       : ");
        String warna = input.bacaString();

        int    tahun = input.bacaInt("  Tahun       : ");
        double harga = input.bacaDouble("  Harga (Rp)  : ");

        if (kategori == 1) {
            double kapasitasMesin  = input.bacaDouble("  Kapasitas Mesin (cc) : ");
            System.out.print("  Jenis BBM             : ");
            String jenisBahanBakar = input.bacaString();
            dealer.tambahMobil(new MobilBensin(id, nama, tipe, warna, tahun, harga,
                    kapasitasMesin, jenisBahanBakar));
        } else {
            double kapasitasBaterai = input.bacaDouble("  Kapasitas Baterai (kWh) : ");
            int    jangkauanKm      = input.bacaInt("  Jangkauan (km)          : ");
            dealer.tambahMobil(new MobilListrik(id, nama, tipe, warna, tahun, harga,
                    kapasitasBaterai, jangkauanKm));
        }

        System.out.println();
        System.out.println("  [OK] Mobil " + nama + " berhasil ditambahkan! ID: " + id);
        garis();
        System.out.println();
    }

    private void menuLihat() {
        System.out.println("  [DAFTAR SEMUA MOBIL]");
        garis();

        ArrayList<Mobil> list = dealer.getDaftarMobil();

        if (list.isEmpty()) {
            System.out.println("  Belum ada data mobil.");
            garis();
            System.out.println();
            return;
        }

        System.out.printf("  %-11s  %-14s  %-18s  %-11s  %-8s  %-6s  %-20s  %-14s%n",
                "ID", "Kategori", "Nama", "Tipe", "Warna", "Tahun", "Harga", "Status");
        garis();

        for (Mobil m : list) {
            String hargaStr  = "Rp " + String.format("%,.0f", m.getHarga());
            String statusStr = m.isTersedia() ? "Tersedia" : "Tidak Tersedia";
            System.out.printf("  %-11s  %-14s  %-18s  %-11s  %-8s  %-6d  %-20s  %-14s%n",
                    m.getId(), m.getKategori(), m.getNama(), m.getTipe(),
                    m.getWarna(), m.getTahun(), hargaStr, statusStr);
        }

        garis();
        System.out.println("  Total: " + list.size() + " unit");
        System.out.println();
    }

    private void menuUbah() {
        System.out.println("  [UBAH DATA MOBIL]");
        garis();

        ArrayList<Mobil> list = dealer.getDaftarMobil();
        if (list.isEmpty()) {
            System.out.println("  Belum ada data mobil untuk diubah.");
            garis();
            System.out.println();
            return;
        }

        System.out.printf("  %-11s  %-14s  %-18s%n", "ID", "Kategori", "Nama");
        garis();
        for (Mobil m : list) {
            System.out.printf("  %-11s  %-14s  %-18s%n",
                    m.getId(), m.getKategori(), m.getNama());
        }
        garis();

        System.out.print("  Masukkan ID yang akan diubah: ");
        String id = input.bacaString();

        Mobil target = dealer.cariMobil(id);
        if (target == null) {
            System.out.println("  [!] Mobil dengan ID " + id + " tidak ditemukan.");
            garis();
            System.out.println();
            return;
        }

        System.out.println();
        System.out.println("  Data saat ini:");
        target.tampilInfo();
        System.out.println();
        System.out.println("  Masukkan data baru (Enter = tidak diubah):");
        System.out.println();

        System.out.print("  Nama Model  [" + target.getNama() + "]: ");
        String nama = input.bacaRaw();
        if (nama.isEmpty()) nama = target.getNama();

        System.out.print("  Tipe Mobil  [" + target.getTipe() + "]: ");
        String tipe = input.bacaRaw();
        if (tipe.isEmpty()) tipe = target.getTipe();

        System.out.print("  Warna       [" + target.getWarna() + "]: ");
        String warna = input.bacaRaw();
        if (warna.isEmpty()) warna = target.getWarna();

        int    tahun = input.bacaIntOpsional("  Tahun       [" + target.getTahun() + "]: ", target.getTahun());
        double harga = input.bacaDoubleOpsional("  Harga (Rp)  [" + (int) target.getHarga() + "]: ", target.getHarga());

        System.out.print("  Status (y=Tersedia, n=Tidak) [" + (target.isTersedia() ? "y" : "n") + "]: ");
        String inputStatus = input.bacaRaw().toLowerCase();
        boolean tersedia = inputStatus.isEmpty() ? target.isTersedia() : inputStatus.equals("y");

        if (target instanceof MobilBensin mb) {
            double mesin = input.bacaDoubleOpsional(
                    "  Kapasitas Mesin (cc) [" + mb.getKapasitasMesin() + "]: ",
                    mb.getKapasitasMesin());
            mb.setKapasitasMesin(mesin);

            System.out.print("  Jenis BBM [" + mb.getJenisBahanBakar() + "]: ");
            String bbm = input.bacaRaw();
            if (!bbm.isEmpty()) mb.setJenisBahanBakar(bbm);
        }

        if (target instanceof MobilListrik ml) {
            double baterai = input.bacaDoubleOpsional(
                    "  Kapasitas Baterai (kWh) [" + ml.getKapasitasBaterai() + "]: ",
                    ml.getKapasitasBaterai());
            ml.setKapasitasBaterai(baterai);

            int jangkauan = input.bacaIntOpsional(
                    "  Jangkauan (km) [" + ml.getJangkauanKm() + "]: ",
                    ml.getJangkauanKm());
            ml.setJangkauanKm(jangkauan);
        }

        boolean sukses = dealer.updateMobil(id, nama, tipe, warna, tahun, harga, tersedia);
        System.out.println();
        System.out.println(sukses
                ? "  [OK] Data mobil " + id + " berhasil diperbarui!"
                : "  [!] Gagal memperbarui data.");
        garis();
        System.out.println();
    }

    private void menuHapus() {
        System.out.println("  [HAPUS DATA MOBIL]");
        garis();

        ArrayList<Mobil> list = dealer.getDaftarMobil();
        if (list.isEmpty()) {
            System.out.println("  Belum ada data mobil untuk dihapus.");
            garis();
            System.out.println();
            return;
        }

        System.out.printf("  %-11s  %-14s  %-18s%n", "ID", "Kategori", "Nama");
        garis();
        for (Mobil m : list) {
            System.out.printf("  %-11s  %-14s  %-18s%n",
                    m.getId(), m.getKategori(), m.getNama());
        }
        garis();

        System.out.print("  Masukkan ID yang akan dihapus: ");
        String id = input.bacaString();

        Mobil target = dealer.cariMobil(id);
        if (target == null) {
            System.out.println("  [!] Mobil dengan ID " + id + " tidak ditemukan.");
            garis();
            System.out.println();
            return;
        }

        System.out.println();
        System.out.println("  Data yang akan dihapus:");
        target.tampilInfo();
        System.out.println();
        System.out.print("  Yakin ingin menghapus? (y/n): ");
        String konfirmasi = input.bacaRaw().toLowerCase();

        if (konfirmasi.equals("y")) {
            dealer.hapusMobil(id);
            System.out.println("  [OK] Mobil " + id + " berhasil dihapus!");
        } else {
            System.out.println("  [!] Penghapusan dibatalkan.");
        }
        garis();
        System.out.println();
    }

    private void menuSimulasi() {
        System.out.println("  [SIMULASI PAJAK & DISKON]");
        garis();

        ArrayList<Mobil> list = dealer.getDaftarMobil();
        if (list.isEmpty()) {
            System.out.println("  Belum ada data mobil.");
            garis();
            System.out.println();
            return;
        }

        System.out.printf("  %-11s  %-14s  %-18s%n", "ID", "Kategori", "Nama");
        garis();
        for (Mobil m : list) {
            System.out.printf("  %-11s  %-14s  %-18s%n",
                    m.getId(), m.getKategori(), m.getNama());
        }
        garis();

        System.out.print("  Masukkan ID mobil: ");
        String id = input.bacaString();

        Mobil m = dealer.cariMobil(id);
        if (m == null) {
            System.out.println("  [!] Mobil tidak ditemukan.");
            garis();
            System.out.println();
            return;
        }

        System.out.println();
        System.out.println("  Mobil     : " + m.getNama() + " (" + m.getKategori() + ")");
        System.out.println("  Harga     : " + String.format("Rp %,.0f", m.getHarga()));
        System.out.println();

        System.out.println("  Pilih jenis diskon:");
        System.out.println("  1. Diskon standar (5%)");
        System.out.println("  2. Diskon dengan persentase sendiri");
        System.out.println("  3. Diskon dengan persentase dan alasan");
        int pilihanDiskon = input.bacaInt("  Pilih [1/2/3]: ");
        System.out.println();

        double diskon = 0;
        if (pilihanDiskon == 1) {
            diskon = m.hitungDiskon();
            System.out.println("  Diskon    : 5% (standar)");
        } else if (pilihanDiskon == 2) {
            double persen = input.bacaDouble("  Masukkan persentase diskon: ");
            diskon = m.hitungDiskon(persen);
            System.out.println("  Diskon    : " + persen + "%");
        } else if (pilihanDiskon == 3) {
            double persen = input.bacaDouble("  Masukkan persentase diskon: ");
            System.out.print("  Masukkan alasan diskon    : ");
            String alasan = input.bacaString();
            diskon = m.hitungDiskon(persen, alasan);
            System.out.println("  Diskon    : " + persen + "%");
        } else {
            System.out.println("  [!] Pilihan tidak valid.");
            garis();
            System.out.println();
            return;
        }

        double pajak       = m.hitungPajak();
        double hargaDiskon = m.getHarga() - diskon;
        double hargaAkhir  = hargaDiskon + pajak;

        System.out.println("  Potongan  : " + String.format("Rp %,.0f", diskon));
        System.out.println("  Harga stlh diskon : " + String.format("Rp %,.0f", hargaDiskon));
        System.out.println("  Pajak     : " + String.format("Rp %,.0f", pajak));
        System.out.println("  Harga akhir       : " + String.format("Rp %,.0f", hargaAkhir));
        garis();
        System.out.println();
    }
}